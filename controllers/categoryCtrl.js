...
const addCategory = async (req, res) => {
    try {
        ...
        const existingCategoryName = await Category.findOne({nameLower: normalizedName});
        if(existingCategoryName){
            return res.render('admin/addCategory', {
                title: 'Add Category',
                errorMessage: 'Category Name is already existed. Please choose another name',
                layout: 'adminlayout'
            })
        }
...
const updateCategory = async (req, res) => {
    try {
        ...
        await Category.findByIdAndUpdate(req.params.id, { name, description, image, nameLower: normalizedName });
